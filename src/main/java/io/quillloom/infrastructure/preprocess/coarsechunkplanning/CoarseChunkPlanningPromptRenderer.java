...
public class CoarseChunkPlanningPromptRenderer {
    ...
    public String render(CoarseChunkPlanningTaskInput input) {
        ...
        builder.append("粗分块默认要明显偏大，优先减少 coarse block 数量，保证后续块级摘要、块级标注和块级知识抽取有稳定上下文。\n");
        builder.append("普通正文只有在章节切换、场景切换、明显时间跳跃、空间跳跃、叙事视角切换时才切出新的 coarse block，不能仅因为换段就切。\n");
        builder.append("标题、题词、诗歌、书信抬头、列表项、署名、版权页这类结构性片段可以短块独立；除此之外，应尽量并入更大的 coarse block。\n");
        builder.append("如果一组相邻段落仍在描写同一连续动作、同一场景或同一叙事单元，必须并入同一个 coarse block。\n");
        builder.append("宁可少切，也不要把整本书切成很长的 coarse block 列表。\n");
